...
class General(commands.Cog, name="General"):
    ...
    async def about(self, context: Context):
        ...
        repository_url = "https://github.com/Kabilan-T/Nanban_bot"
        embed = discord.Embed(
            title="About",
            description = (
                f"**{self.bot.name}** is a Discord bot developed as part of the "
                f"[Nanban bot Project]({repository_url}). "
                ...
                f"To know more, visit the repository here: [Nanban_bot]({repository_url}?tab=readme-ov-file#nanban_bot)"
            ),
            color=self.bot.default_color,
        )
        await context.send(embed=embed)
